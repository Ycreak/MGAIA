...
def _find_topic_options(state, topic):
...
def _render_topic_options(state):
    i = 0

    for option in state.topic_options:
        buttonpos = "topic_option_button" + str(i)

        if len(option) > 44:
            option = option[:45] + "..."

        state.render_button(state.positions[buttonpos],
                            option,
                            state.smallfont,
                            state.colors["buttontext"],
                            [10, 10],
                            state.colors["color_light"],
                            state.colors["color_dark"])

        i = i + 1


def _find_subtopics(state, chosen_topic):
